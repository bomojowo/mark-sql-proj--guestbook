...
app.get("/signatures", async (req, res) => {
  const dbResult = await client.query("SELECT id, signature, message FROM signatures ORDER BY id");
...
app.post("/signatures", async (req, res) => {
  ...
  const text = "INSERT INTO signatures(signature, message) VALUES ($1, $2)"
...
app.put("/signatures/:id", async (req, res) => {
  ...
  const text = "UPDATE signatures SET signature = $1 WHERE id = $2"
